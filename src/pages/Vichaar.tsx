    ...
    <div className="min-h-full p-6 pb-24 bg-transparent text-gray-900 dark:text-gray-100 transition-colors duration-300 flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center relative w-full">
        ...
        <div className="relative group w-full max-w-md aspect-square">
          <div className={cn(
            "absolute -inset-1 rounded-[3rem] blur-xl opacity-35 group-hover:opacity-60 transition duration-1000 bg-gradient-to-br",
            quote.color
          )} />
          
          <div className={cn(
            "w-full h-full rounded-[3rem] p-8 sm:p-10 flex flex-col justify-center relative overflow-hidden transition-all duration-700 border border-gray-200 dark:border-white/10",
            "bg-white/95 dark:bg-[#121212]/90 backdrop-blur-2xl shadow-xl dark:shadow-[0_0_30px_rgba(0,0,0,0.8)]"
          )}>
            {/* Decorative Elements */}
            <div className={cn("absolute top-0 left-0 w-full h-full opacity-25 dark:opacity-20 bg-gradient-to-br mix-blend-overlay", quote.color)} />
            <Quote size={160} className="absolute -top-10 -left-10 text-gray-300/10 dark:text-white/5 rotate-180 drop-shadow-2xl" />
            <Quote size={160} className="absolute -bottom-10 -right-10 text-gray-300/10 dark:text-white/5 drop-shadow-2xl" />
            
            <div className="relative z-10 text-center space-y-8">
              <h2 className="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white leading-tight drop-shadow-none dark:drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)] tracking-wide">
                "{quote.hi}"
              </h2>
              ...
              <p className="text-lg sm:text-xl text-gray-700 dark:text-gray-300 font-bold drop-shadow-none dark:drop-shadow-md leading-relaxed">
                "{quote.en}"
              </p>
              
              <div className="pt-4">
                <p className="inline-block px-4 py-1.5 bg-gray-100 dark:bg-white/5 backdrop-blur-md rounded-full border border-gray-200 dark:border-white/10 text-gray-700 dark:text-[#FFD54F] font-black tracking-widest uppercase text-[10px] shadow-none dark:shadow-[0_0_15px_rgba(0,0,0,0.5)]">
                  {quote.source}
                </p>
              </div>
            </div>
        ...
        {/* Navigation Controls */}
        <button onClick={prevQuote} className="absolute left-0 sm:-left-4 p-4 bg-white dark:bg-[#1A1A1A]/80 backdrop-blur-xl rounded-full shadow-md dark:shadow-[0_0_20px_rgba(0,0,0,0.5)] border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:text-[#FF6D00] dark:hover:text-[#FFD54F] hover:border-[#FFD54F]/30 hover:scale-110 transition-all duration-300 z-20">
          <ChevronLeft size={28} />
        </button>
        <button onClick={nextQuote} className="absolute right-0 sm:-right-4 p-4 bg-white dark:bg-[#1A1A1A]/80 backdrop-blur-xl rounded-full shadow-md dark:shadow-[0_0_20px_rgba(0,0,0,0.5)] border border-gray-200 dark:border-white/10 text-gray-500 dark:text-gray-400 hover:text-[#FF6D00] dark:hover:text-[#FFD54F] hover:border-[#FFD54F]/30 hover:scale-110 transition-all duration-300 z-20">
      ...
      <div className="flex justify-center gap-8 mt-12 shrink-0 relative z-20">
        <button 
          onClick={toggleLike}
          className={cn(
            "w-16 h-16 rounded-full shadow-md dark:shadow-[0_0_20px_rgba(0,0,0,0.5)] flex items-center justify-center hover:scale-110 transition-all duration-300 border",
            liked[currentIndex] 
              ? "bg-[#F50057]/10 border-[#F50057]/30 shadow-[0_0_20px_rgba(245,0,87,0.2)]" 
              : "bg-white dark:bg-[#1A1A1A]/80 border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/30 backdrop-blur-xl"
          )}
        >
          <Heart size={28} className={cn("transition-colors duration-300", liked[currentIndex] ? "fill-[#F50057] text-[#F50057] drop-shadow-[0_0_8px_rgba(245,0,87,0.8)]" : "text-gray-500 dark:text-gray-400")} />
        </button>
        <button 
          onClick={handleCopy}
          className="w-16 h-16 bg-white dark:bg-[#1A1A1A]/80 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-full shadow-md dark:shadow-[0_0_20px_rgba(0,0,0,0.5)] flex items-center justify-center hover:scale-110 hover:border-[#00E676]/30 hover:bg-[#00E676]/10 transition-all duration-300 text-gray-500 dark:text-gray-400 hover:text-[#00C853] dark:hover:text-[#69F0AE] group"
        >
          {copied ? <Check size={28} className="text-[#00C853] dark:text-[#00E676] drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(0,230,118,0.8)]" /> : <Copy size={28} className="group-hover:drop-shadow-none dark:group-hover:drop-shadow-[0_0_8px_rgba(105,240,174,0.8)]" />}
        </button>
        <button 
          onClick={handleShare}
          className="w-16 h-16 bg-white dark:bg-[#1A1A1A]/80 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-full shadow-md dark:shadow-[0_0_20px_rgba(0,0,0,0.5)] flex items-center justify-center hover:scale-110 hover:border-[#2962FF]/30 hover:bg-[#2962FF]/10 transition-all duration-300 text-gray-500 dark:text-gray-400 hover:text-[#2962FF] dark:hover:text-[#82B1FF] group"
        >
          <Share2 size={28} className="group-hover:drop-shadow-none dark:group-hover:drop-shadow-[0_0_8px_rgba(130,177,255,0.8)]" />
        </button>
      </div>
          ...
          <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-white/10 rounded-[2rem] w-full max-w-lg p-6 md:p-8 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh] text-gray-900 dark:text-gray-100 transition-colors duration-300">
            ...
            <div className="flex justify-between items-start mb-5 relative z-10">
              <div className="text-left">
                <span className="text-[9px] font-black text-[#FF6D00] uppercase tracking-widest bg-[#FF6D00]/10 px-3 py-1 rounded-full border border-[#FF6D00]/20 inline-block mb-1.5 font-mono">
            ...
            <div className="overflow-y-auto pr-1 space-y-4 text-left text-gray-650 dark:text-gray-300 text-xs leading-relaxed relative z-10 max-h-[55vh]">
              ...
              <ul className="list-disc pl-5 space-y-2 text-gray-500 dark:text-gray-400 font-semibold font-sans">
                <li>
                  <strong className="text-gray-900 dark:text-[#FFD54F]">{language === 'en' ? 'Daily Up-to-date Quotes:' : 'नित्य नए चिंतनशील विचार:'}</strong>{' '}
                  {language === 'en' 
                ...
                <li>
                  <strong className="text-gray-900 dark:text-[#FFD54F]">{language === 'en' ? 'Copy & Share Instantly:' : 'कॉपी और सीधा साझा करें:'}</strong>{' '}
                  {language === 'en' 
                ...
                <li>
                  <strong className="text-gray-900 dark:text-[#FFD54F]">{language === 'en' ? 'Lineages Respect:' : 'सभी संप्रदायों का आदर:'}</strong>{' '}
                  {language === 'en' 
