...
export default function ChatPage() {
  const { user, login, loginWithEmail, registerWithEmail, loginAsDemo, logout, error: authError, setError: setAuthError } = useAuth();
        ...
        <div className="bg-white/80 dark:bg-[#121212]/80 backdrop-blur-md p-10 rounded-[2.5rem] border border-gray-200 dark:border-white/10 w-full max-w-lg text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-600 dark:text-gray-400 max-w-sm mx-auto mb-6 text-sm leading-relaxed font-semibold">
              {!showEmailLogin 
                ? 'Please authenticate via your Google Account or custom Email & Password to access secure spiritual guidance.'
                : 'Enter your Email & Password to securely log in or register from your mobile app/WebView.'}
            </p>
              ...
              <div className="mb-6 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-800 dark:text-amber-300 text-xs font-semibold text-left space-y-2 relative">
                ...
                <p className="leading-relaxed font-semibold text-gray-600 dark:text-gray-300">
                  {authError.includes('unauthorized-domain') 
                    ? `This domain (${window.location.hostname}) is not whitelisted in your Firebase console. To log in with Google, add this domain under Authentication > Settings > Authorized Domains. Alternately, use the secure custom Email & Password login below.`
                    : authError}
                </p>
                {!showEmailLogin && (
                  <button
                  ...
                  >
                    Use Custom Email & Password Login / रजिस्टर करें
                  </button>
                )}
              </div>
            )}
            ...
            {!showEmailLogin ? (
              <>
                <button
                  onClick={login}
                  className="w-full py-4 bg-gray-100 dark:bg-white hover:bg-gray-200 dark:hover:bg-gray-100 text-gray-900 dark:text-black rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 shadow-lg hover:scale-105 active:scale-95 transition-all duration-300 mb-4"
                >
                ...
                <div className="flex items-center my-5">
                  ...
                  <span className="px-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest">OR / अथवा</span>
                  <div className="flex-1 border-t border-gray-200 dark:border-white/10"></div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-gradient-to-r from-[#FF6D00] to-[#FFD54F] text-black rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:scale-105 active:scale-95 transition-all duration-300"
                >
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    if (isEmailSignUp) {
                      ...
                      await registerWithEmail(emailVal, passwordVal, nameVal);
                    } else {
              ...
              >
                <h3 className="text-xl font-bold text-gray-900 dark:text-white text-center uppercase tracking-tight">
                  {isEmailSignUp ? 'Create Account / नया अकाउंट' : 'Email Sign In / लॉगिन करें'}
                </h3>

                {isEmailSignUp && (
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Full Name / नाम</label>
                    <input 
                      type="text" 
                      required
                      value={nameVal}
                      onChange={(e) => setNameVal(e.target.value)}
                      className="w-full p-4 rounded-2xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 font-medium focus:ring-2 focus:ring-[#FF6D00] outline-none transition-all dark:text-white text-gray-900" 
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Email / ईमेल</label>
                  <input 
                    type="email" 
                    required
                    value={emailVal}
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">Password / पासवर्ड</label>
                  <input 
                    type="password" 
                    required
                    minLength={6}
                ...
                <button
                  type="submit"
                  className="w-full py-4 mt-2 bg-gradient-to-r from-[#FF6D00] to-[#FFD54F] text-black rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:scale-105 active:scale-95 transition-all duration-300"
                >
                  {isEmailSignUp ? 'Complete Registration / अकाउंट बनाएं' : 'Sign In / लॉगिन करें'}
                </button>
                ...
                <div className="flex flex-col gap-3 text-center pt-2">
                  ...
                  >
                    {isEmailSignUp ? 'Already have an account? Sign In / लॉगिन करें' : 'Create new account / नया अकाउंट बनाएं'}
                  </button>
                  ...
                  >
                    ← Back to Google Sign-In / गूगल लॉगिन पर जाएं
                  </button>
                </div>
              </form>
            )}
