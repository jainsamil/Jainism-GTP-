...
export default function SaintsPage() {
  ...
  const displaySaints = useMemo(() => {
    const deletedNames = new Set(
      saints
        .filter((fs: any) => fs.deleted === true)
        .map((fs: any) => fs.name?.en?.toLowerCase())
        .filter(Boolean)
    );
    const deletedIds = new Set(saints.filter((fs: any) => fs.deleted === true).map((fs: any) => fs.id));
    
    const activeSaints = saints.filter((fs: any) => fs.deleted !== true);
    
    const combined = FALLBACK_SAINTS.filter(
      item => !deletedNames.has(item.name?.en?.toLowerCase())
    );
    
    activeSaints.forEach((fs: any) => {
